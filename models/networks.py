...
import numpy as np
from .NS_networks import ResnetGeneratorWith1DConv
import numbers
import math
###############################################################################
# Helper Functions
...
def get_norm_layer(norm_type='instance'):
    ...
    if norm_type == 'batch':
        ...
        norm_layer = functools.partial(nn.InstanceNorm2d, affine=False, track_running_stats=False)  # no parameters
    elif norm_type == 'none':
        def norm_layer(x):
            return Identity()
    else:
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    if netG == 'resnet_9blocks':
        ...
        net = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=6, opt=opt, activation_type='sigmoid')
    elif netG == 'resnet_4blocks':
        net = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=4, opt=opt, activation_type='sigmoid')
    elif netG == 'NS_resnet_2down_4blocks_1dConv':
        net = ResnetGeneratorWith1DConv(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                              no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=opt.NS_n_RBlock, opt=opt, activation_type='none')
    elif netG == 'unet_128':
...
class GANLoss(nn.Module):
    ...
    def __init__(self, gan_mode, target_real_label=1.0, target_fake_label=0.0, use_mask=False):
        ...
        if gan_mode == 'lsgan':
            if use_mask:
                self.loss = nn.MSELoss(reduction='sum')  # no reduction
            else:
    ...
    def __call__(self, prediction, target_is_real, mask_tensor=None):
        ...
        if self.gan_mode in ['lsgan', 'vanilla']:
            if self.use_mask:
                ...
                loss = self.loss(prediction * mask_tensor, target_tensor * mask_tensor) / torch.sum(mask_tensor)
            else:
                target_tensor = self.get_target_tensor(prediction, target_is_real)
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None, activation_type='tanh'):
        ...
        model += [nn.Conv2d(input_nc, ngf, kernel_size=7, padding=0, bias=use_bias), norm_layer(ngf), nn.ReLU(True)]  # 1, 2, 3
        ...
        for i in range(n_downsampling):  # add downsampling layers
            ...
            if(no_antialias):
                model += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias),
                          ...
                          nn.ReLU(True)]  # 4, 5, 6    7, 8, 9
            else:
                model += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=1, padding=1, bias=use_bias),
                          norm_layer(ngf * mult * 2),
                          nn.ReLU(True),
                          Downsample(ngf * mult * 2)]  # 4, 5, 6, 7,  8, 9, 10, 11
...
class Whole64Discriminator(nn.Module):  # no padding
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False):

        super(Whole64Discriminator, self).__init__()
        if type(norm_layer) == functools.partial:  # no need to use bias as BatchNorm2d has affine parameters
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
        ...
        if(no_antialias):
            sequence = [nn.Conv2d(input_nc, ndf, kernel_size=kw, stride=2), nn.LeakyReLU(0.2, False)]
        else:
            sequence = [nn.Conv2d(input_nc, ndf, kernel_size=kw, stride=1), nn.LeakyReLU(0.2, False), Downsample(ndf)]
        nf_mult = 1
        nf_mult_prev = 1
        for n in range(1, n_layers):  # gradually increase the number of filters
            nf_mult_prev = nf_mult
        ...
        sequence += [nn.Conv2d(ndf * nf_mult, 1, kernel_size=3, stride=1)]  # output 1 channel prediction map
        self.model = nn.Sequential(*sequence)
...
class AdaSingleDiscriminator(nn.Module):  # no padding
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False):

        super(AdaSingleDiscriminator, self).__init__()
        if type(norm_layer) == functools.partial:  # no need to use bias as BatchNorm2d has affine parameters
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
        ...
        sequence += [nn.AdaptiveAvgPool2d(1), nn.Conv2d(ndf * nf_mult, 1, kernel_size=1, stride=1)]  # output 1 channel prediction map
        self.model = nn.Sequential(*sequence)
...
class Sin11Discriminator(nn.Module):  # no padding
    def __init__(self, input_nc, ndf=64):

        super(Sin11Discriminator, self).__init__()
        sequence = []

        sequence += [
            nn.Conv2d(input_nc, ndf, kernel_size=3, stride=1),
            nn.BatchNorm2d(ndf),
            nn.LeakyReLU(0.2, False)
        ]
        for layer_id in range(3):
            sequence += [
                nn.Conv2d(ndf, ndf, kernel_size=3, stride=1),
                nn.BatchNorm2d(ndf),
                nn.LeakyReLU(0.2, False)
            ]

        sequence += [nn.Conv2d(ndf, 1, kernel_size=3, stride=1)]  # output 1 channel prediction map
        self.model = nn.Sequential(*sequence)
