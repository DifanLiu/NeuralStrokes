...
def get_transform(opt, params=None, grayscale=False, method=Image.BICUBIC, convert=True):
    ...
    if 'fixsize' in opt.preprocess:  # false
        transform_list.append(transforms.Resize(params["size"], method))
    if 'resize' in opt.preprocess:  # false
        osize = [opt.load_size, opt.load_size]
        if "gta2cityscapes" in opt.dataroot:
            osize[0] = opt.load_size // 2
        transform_list.append(transforms.Resize(osize, method))
    elif 'scale_width' in opt.preprocess:  # false
        transform_list.append(transforms.Lambda(lambda img: __scale_width(img, opt.load_size, opt.crop_size, method)))
    elif 'scale_shortside' in opt.preprocess:  # false
    ...
    transform_list.append(transforms.Lambda(lambda img: __make_power_2(img, base=4, method=method)))  # make sure that crop size is 4N

    if not opt.no_flip:
        if params is None or 'flip' not in params:
            transform_list.append(transforms.RandomHorizontalFlip())
        elif 'flip' in params:  # True 3
            transform_list.append(transforms.Lambda(lambda img: __flip(img, params['flip'])))  # flip left and right
...
def __patch(img, index, size):  # TODO     256 - 768    crop_size 64
    ow, oh = img.size
    nw, nh = ow // size, oh // size  # floor
    roomx = ow - nw * size  # remainder
    roomy = oh - nh * size
    startx = np.random.randint(int(roomx) + 1)  # discrete uniform
    starty = np.random.randint(int(roomy) + 1)

    index = index % (nw * nh)
    ix = index // nh
    iy = index % nh  # select a grid
